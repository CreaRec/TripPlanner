...
import { getItinerary, type ItineraryDayWithItems } from "../trip/itinerary";
import { listPlaces } from "../places/places";
...
export async function exportItineraryPdf(
  ...
  const fingerprint = computeItineraryExportFingerprint(trip, itinerary, places, ExportFormat.Pdf);
  const s3Key = itineraryExportKey(trip.id, ExportFormat.Pdf);
...
export async function exportItineraryCsv(
  ...
  const itinerary = await getItinerary(trip.id);
  const places = await listPlaces(trip.id);
  const fingerprint = computeItineraryExportFingerprint(trip, itinerary, places, ExportFormat.Csv);
  const s3Key = itineraryExportKey(trip.id, ExportFormat.Csv);
