...
const EXPORT_BY_NUMBER =
  /^(?:(?:дай|покажи|открой|экспортируй|export|open|show|get|send|обнови|refresh|пересоздай|re-?export|update)\s+)?(?:мне\s+)?(?:письмо|email|e-?mail|mail|message)\s*#?\s*(\d+)\s*[.!?]*$/iu;
const REFRESH_VERBS =
  /(?:^|\s)(?:обнови|refresh|пересоздай|re-?export|update)(?:\s|$)/iu;

export interface ExportGmailByNumberRequest {
  index: number;
...
export function parseExportGmailByNumberRequest(text: string): ExportGmailByNumberRequest | null {
  const trimmed = text.trim();
  ...
  return { index, forceRefresh: REFRESH_VERBS.test(trimmed) };
}
