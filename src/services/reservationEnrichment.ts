...
import { EnrichmentProvider, parseEnrichmentProvider } from "./enrichmentProvider";
import type { AviationStackAirport, AviationStackFlight } from "./aviationStack";
...
function applyAviationStackAirport(
  ...
  if (changed) {
    meta.enrichment_provider = EnrichmentProvider.AviationStack;
  }
  return changed;
}
...
async function enrichAirportViaGoogle(
  ...
  if (enriched && meta.enrichment_provider !== EnrichmentProvider.AviationStack) {
    meta.enrichment_provider = EnrichmentProvider.GooglePlaces;
  }
...
async function enrichPlaceReservation(
  ...
  return {
    enriched,
    address,
    enrichmentProvider: enriched ? EnrichmentProvider.GooglePlaces : null,
    telemetry,
  };
}
...
export async function reEnrichFlightReservations(
  ...
  return reEnrichReservations(reservations, destinationByTripId, { types: ["flight"] });
}
...
export function summarizeReEnrichResults(results: ReEnrichReservationResult[]) {
  return {
    ...
    aviationstack_used_count: results.filter(
      (result) => result.enrichmentProvider === EnrichmentProvider.AviationStack,
    ).length,
    google_places_used_count: results.filter(
      (result) => result.enrichmentProvider === EnrichmentProvider.GooglePlaces,
    ).length,
