    ...
    expect(m.addReservation).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        metadata: expect.objectContaining({
          ...
          enrichment_provider: EnrichmentProvider.AviationStack,
        }),
      }),
    );
    expect(result.enriched).toBe(true);
...
describe("reEnrichReservation", () => {
  ...
  it("force-refreshes flight metadata and reports aviation stack telemetry", async () => {
    ...
    m.updateReservation.mockResolvedValueOnce({
      ...
      metadata: {
        ...
        route: "AUS -> SEA",
        flight_status: "scheduled",
        enrichment_provider: "aviationstack",
      },
    });
    ...
    expect(result.enriched).toBe(true);
    expect(result.enrichmentProvider).toBe(EnrichmentProvider.AviationStack);
  ...
  it("force-refreshes hotel metadata via Google Places", async () => {
    ...
    m.updateReservation.mockResolvedValueOnce({
      ...
      notes: null,
      metadata: {
        enrichment_provider: EnrichmentProvider.GooglePlaces,
    ...
    expect(result.enriched).toBe(true);
    expect(result.enrichmentProvider).toBe(EnrichmentProvider.GooglePlaces);
