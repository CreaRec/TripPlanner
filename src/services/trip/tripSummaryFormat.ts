...
function shouldMergeReservations(a: Reservation, b: Reservation): boolean {
  ...
  if (a.type === ReservationType.Flight) {
  ...
  if (a.type === ReservationType.Hotel || a.type === ReservationType.CarRental) {
...
function memoryDuplicatesReservation(content: string, reservations: Reservation[]): boolean {
  ...
  for (const reservation of reservations) {
    ...
    if (reservation.type === ReservationType.CarRental && /car rental|аренд|rental car/i.test(content)) {
      const titlePrefix = reservation.title.trim().toLowerCase().slice(0, 12);
      if (titlePrefix && lower.includes(titlePrefix)) return true;
    }

    if (reservation.type === ReservationType.Hotel && /hotel|отель|hyatt|marriott|hilton/i.test(content)) {
      const titlePrefix = reservation.title.trim().toLowerCase().slice(0, 12);
      if (titlePrefix && lower.includes(titlePrefix)) return true;
    }
  }
...
function reservationIcon(type: string): string {
  switch (type) {
    ...
    case ReservationType.Hotel:
      return "🏨";
    case ReservationType.CarRental:
      return "🚗";
    case ReservationType.Campsite:
      return "⛺";
    default:
      return "📌";
  }
...
function reservationDayEvents(
  ...
  if (reservation.type === ReservationType.Hotel) {
    if (start === dayDate) {
      events.push({
  ...
  if (reservation.type === ReservationType.CarRental) {
    if (start === dayDate) {
      events.push({
...
function formatCardSummary(input: FormatTripSummaryInput): string {
  ...
  const flights = reservations.filter((r) => r.type === ReservationType.Flight);
  const cars = reservations.filter((r) => r.type === ReservationType.CarRental);
  const hotels = reservations.filter((r) => r.type === ReservationType.Hotel);
  const other = reservations.filter(
    (r) =>
      r.type !== ReservationType.Flight &&
      r.type !== ReservationType.CarRental &&
      r.type !== ReservationType.Hotel,
  );
